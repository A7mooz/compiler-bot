import { BaseInteraction, Message, roleMention, type Interaction } from 'discord.js';

import { type AnyCommand } from './index.js';
...
export default function (ctx: Message | Interaction, command: AnyCommand) {
	...
	if (command.guildOwnerOnly && member.user.id !== ctx.guild?.ownerId) return 'This command is only for the server owner.';
