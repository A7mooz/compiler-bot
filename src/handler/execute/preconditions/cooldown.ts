import { Collection, type Interaction, Message, time } from 'discord.js';
import ms from 'ms';
...
export default function (ctx: Message | Interaction, command: AnyCommand) {
	...
	if (cooldown && cooldown > new Date()) return `You are being too fast, you can reuse the command ${time(cooldown, 'R')}`;
