...
export default new Listener({
	...
	execute(message, instance) {
		...
		const command = <Command<CommandType.Text> | undefined>(
			instance.commands.find((cmd) => (cmd.name === name || cmd.alias.includes(name)) && cmd.type.includes(CommandType.Text))
		);
		...
		const send: Send<CommandType.Text, CacheType> = (options) => {
			...
			const reply = message.channel.messages.cache.find((v) => v.reference?.messageId === message.id && v.author.id === client.user.id);
