import type { CommandOptions, Execute } from '#types/command';
...
export class Command<T extends CommandType = CommandType> {
	...
	constructor(data: CommandOptions<T>) {
		...
		this.defaultMemberPermissions = `${this.permissions.bitfield || PermissionFlagsBits.ViewChannel}`;

		if (data.defaultMemberPermissions) this.defaultMemberPermissions = `${PermissionsBitField.resolve(data.defaultMemberPermissions || 0n)}`;
		...
		if ('contextMenuExec' in data) this.contextMenuExec = data.contextMenuExec as Command<T>['contextMenuExec'];
		if ('autocompleteExec' in data) this.autocompleteExec = data.autocompleteExec as Command<T>['autocompleteExec'];
	}
