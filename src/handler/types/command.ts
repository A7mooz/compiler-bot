...
import type { CommandType, CustomInstance } from '#handler';
...
type InteractionType<T extends Types, Cached extends DJS.CacheType> = T extends keyof InteractionMap<Cached> ? InteractionMap<Cached>[T] : never;
...
type SendOptions<T extends CommandType> = T extends CommandType.Text
	...
	: (DJS.InteractionReplyOptions & { new: true }) | (DJS.InteractionEditReplyOptions & { new?: false });

type Send<T extends CommandType, Cached extends DJS.CacheType> = (options: SendOptions<T> | string) => Promise<DJS.Message<DJS.BooleanCache<Cached>>>;
...
type Execute<T extends Types, Cached extends DJS.CacheType> = (context: Context<T, Cached>) => DJS.Awaitable<void>;

type TypedExec<T extends Types, Cached extends DJS.CacheType, Extends extends CommandType, Type extends Types = Extends> = T extends Extends
	? Execute<Type, Cached>
	: never;
...
interface BaseCommandOptions<T extends CommandType, Cached extends DJS.CacheType> {
	...
	description: string;
	category?: string;
	/** Command usage cooldown a number without a unit means `ms`*/
	cooldown?: number | `${number}${'' | 'ms' | 's' | 'm' | 'h' | 'd'}`;
	...
	contextMenuExec?: TypedExec<T, Cached, Extract<T, CommandType.MessageMenu | CommandType.UserMenu>>;
...
interface DMCommandOptions<T extends CommandType> extends BaseCommandOptions<T, 'cached' | undefined> {
	dmPermission: true;
}
...
export type { CommandOptions, Execute, InteractionTypes, Send };
